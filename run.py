...
if __name__ == "__main__":
    ...
    title = f"Plan for {args.planner} planner with params:\n"
    if args.planner == 'rrt':
        title += f" ext_mode={args.ext_mode}, goal_prob={args.goal_prob}"
    elif args.planner == 'rrtstar':
        if args.k == -1:
            title += f" ext_mode={args.ext_mode}, goal_prob={args.goal_prob}, k=logn"
        else:
            title += f" ext_mode={args.ext_mode}, goal_prob={args.goal_prob}, k={args.k}"
    if args.planner == 'astar':
        planner.planning_env.visualize_map(plan=plan, expanded_nodes=planner.get_expanded_nodes(), title=title)
    else:
        planner.planning_env.visualize_map(plan=plan, tree_edges=planner.tree.get_edges_as_states(), title=title)